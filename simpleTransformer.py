...
max_iters = 5000 #5000
eval_interval = 100 # how often to get validation and training error.
learning_rate = 2e-3 #1e-3
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 200
n_embd = 64 # size of linear transformations
n_head = 4 # number of attention heads
n_layer = 4 # number of blocks
dropout = 0.0 # dropout samples to avoid overfitting
# ------------
...
for iter in range(max_iters):
    ...
    if iter % (eval_interval*5) == 0:
        context = torch.tensor([encode("Hermione saw ")], dtype=torch.long)
...
# generate from the model, no prompt given
context = torch.zeros((1, 1), dtype=torch.long, device=device)
print(decode(m.generate(context, max_new_tokens=2000)[0].tolist()))

# example usage with an initial prompt.
